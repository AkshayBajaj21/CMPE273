...
export class Courseinfo extends Component {
  ...
  submitHandler = (e) => {
    ...
    axios.post(`http://localhost:3001/course/${this.state.cid}/information`,data)
    .then((response)=>{
  ...
  componentDidMount(){
    console.log("entered course info CDM " + this.props.match.params.id);
     ...
     axios.get(`http://localhost:3001/course/${this.state.cid}/information`)
     .then((response) => {
       console.log("information returned in frontend : " + response.data.message);
       this.setState({      
        information : response.data.information[0],
        statinfo : response.data.status
        });
